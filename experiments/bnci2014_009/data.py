...
from experiments.bnci2014_009.config import BNCI009_LABELS, BNCI2014009Config

BNCI009SampleKey = tuple[int, str, str, int]


@dataclass(frozen=True, slots=True)
class BNCI009EpochMetadata:
    subject: int
    session: str
    run: str
    epoch_index: int
    ...
    @property
    def sample_key(self) -> BNCI009SampleKey:
        return (self.subject, self.session, self.run, self.epoch_index)


@dataclass(frozen=True, slots=True)
class BNCI009EpochDataset:
    X: NDArray[np.floating[Any]]
    y: NDArray[np.integer[Any]]
    metadata: tuple[BNCI009EpochMetadata, ...]
    ...
    @property
    def sample_keys(self) -> tuple[BNCI009SampleKey, ...]:
...
@dataclass(frozen=True, slots=True)
class BNCI009SplitAudit:
    split_name: str
    overlapping_subjects: tuple[int, ...]
    overlapping_sample_keys: tuple[BNCI009SampleKey, ...]
    ...
    @property
    def train_target_fraction(self) -> float:
        return _positive_fraction(self.train_class_counts)

    @property
    def test_target_fraction(self) -> float:
        return _positive_fraction(self.test_class_counts)
...
def build_epoch_dataset(
    ...
    *,
    class_names: tuple[str, ...] = BNCI009_LABELS,
    ...
    if unknown_labels:
        raise ValueError(f"Unknown BNCI2014_009 labels: {unknown_labels}")
...
def create_leave_one_subject_splits(dataset: BNCI009EpochDataset) -> tuple[BNCI009Split, ...]:
...
def audit_split(dataset: BNCI009EpochDataset, split: BNCI009Split) -> BNCI009SplitAudit:
...
def _positive_fraction(counts: dict[str, int]) -> float:
